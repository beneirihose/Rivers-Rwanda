    ...
    <div>
      <div className="relative aspect-[16/10] rounded-xl overflow-hidden cursor-pointer group bg-gray-100" onClick={() => openModal(currentIndex)}>
        <AnimatePresence initial={false}>
            <motion.img
                key={currentIndex}
                src={`${import.meta.env.VITE_API_URL?.replace('/api/v1', '') || 'https://rivers-rwanda.onrender.com'}${images[currentIndex]}`}
      ...
      <div className="grid grid-cols-6 gap-2 mt-2">
        {images.slice(0, 6).map((image, index) => (
          <div 
            ...
            <img src={`${import.meta.env.VITE_API_URL?.replace('/api/v1', '') || 'https://rivers-rwanda.onrender.com'}${image}`} alt={`Thumbnail ${index + 1}`} className="w-full h-full object-cover" />
          </div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          ...
          >
            ...
            >
                <img src={`${import.meta.env.VITE_API_URL?.replace('/api/v1', '') || 'https://rivers-rwanda.onrender.com'}${images[currentIndex]}`} className="w-full h-full object-contain"/>
